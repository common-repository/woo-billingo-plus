...
if ( ! class_exists( 'WC_Billingo_Plus_Pro', false ) ) :

	class WC_Billingo_Plus_Pro {
		...
		public static function pro_activate($pro_key = false) {
			check_ajax_referer( 'wc-billingo-plus-license-check', 'nonce' );
			...
			if( is_wp_error( $response ) ) {
				wp_send_json_error(array(
					'message' => __('Unable to activate the PRO version. Please make sure that the entered data is correct.', 'wc-billingo-plus')
				));
			}
		...
		public static function pro_deactivate() {
			...
			if( is_wp_error( $response ) ) {
				wp_send_json_error(array(
					'message' => __('Unable to deactivate the PRO version. Please make sure that the entered data is correct.', 'wc-billingo-plus')
				));
			}
		...
		public static function migrate_old_pro() {
			if(get_option('_wc_billingo_pro_enabled') && get_option('_wc_billingo_pro_key')) {

				//Migrate into new names
				$key = get_option('_wc_billingo_pro_key');
				update_option('_'.self::$id.'_pro_key', $key);
				update_option('_'.self::$id.'_pro_enabled', true);
				update_option('_'.self::$id.'_pro_meta', array());

				//Delete old stuff, so this won't run again
