...
if ( ! class_exists( 'WC_Billingo_Plus_Grouped_Invoice', false ) ) :

	class WC_Billingo_Plus_Grouped_Invoice {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wc_billingo_plus_generate_grouped_invoice', array( __CLASS__, 'generate_grouped_invoice' ) );
		}

		public static function add_bulk_options( $actions ) {
			$actions['wc_billingo_plus_bulk_grouped_generate'] = __( 'Create Billingo combined invoice', 'wc-billingo-plus' );
			return $actions;
		}
		...
		public static function generate_grouped_invoice() {
			check_ajax_referer( 'wc_billingo_plus_generate_grouped_invoice', 'nonce' );
			...
			$order_status = str_replace( 'wc-', '', WC_Billingo_Plus()->get_option('grouped_invoice_status', 'no') );
			...
			if(WC_Billingo_Plus()->is_invoice_generated($main_order_id, 'invoice')) {
				$response['error'] = true;
				$response['messages'][] = sprintf( esc_html__( 'An invoice has been already created for the selected orders(#%1$s).', 'wc-billingo-plus' ), $main_order->get_order_number() );
				wp_send_json_success($response);
				return false;
			}

			//Create an invoice(multiple orders passed in the first parameter)
			$xml_response = WC_Billingo_Plus()->generate_invoice($order_ids, 'invoice');
			...
			if(!$xml_response['error']) {
				...
				foreach ($order_ids as $order_id) {
					...
					if($order_id == $main_order_id) {
						$order->add_order_note(sprintf( esc_html__( 'A combined invoice has been created for this order. This invoice also has line items from the following orders: #%1$s', 'wc-billingo-plus' ), implode(', ', $order_numbers) ));
					}

					//If its not the main order, disable the auto invoice generation and store where to find the actual invoice
					if($order_id != $main_order_id) {
						$order->update_meta_data('_wc_billingo_plus_own', sprintf( esc_html__( 'A combined invoice has been created for this order. You can find the invoice on the main order: #%1$s', 'wc-billingo-plus' ), $order->get_order_number() ));
					}
